...
def load_fashion_mnist_datasets(val_ratio=0.2, num_train_examples=None, seed=42, data_dir=None):

    if data_dir is None:
        data_dir = os.path.join(os.environ['DATA_DIR'], 'fashion-mnist')
    ...
    train_val_data = datasets.FashionMNIST(data_dir, train=True, download=True, transform=composed_transform)
    test_data = datasets.FashionMNIST(data_dir, train=False, download=True, transform=composed_transform)
    ...
    for dataset in [train_data, val_data, test_data]:
        dataset.dataset_name = 'fashion_mnist'
        dataset.statistics = (means, stds)

    return train_data, val_data, test_data, None


def load_fashion_mnist_loaders(val_ratio=0.2, batch_size=128, seed=42, drop_last=False, num_train_examples=None):
    train_data, val_data, test_data, info = load_fashion_mnist_datasets(
